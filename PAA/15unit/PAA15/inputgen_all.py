...
def generate(num_confs):
    for i in range(num_confs):
        ...
        subprocess.run(["python","inputgen.py","--method=auto","conf"+str(i)+"_edited.pdb"])

def run(num_confs):
    for i in range(num_confs): 
        print( "Running polar conf ", i+1)
        ...
        for i in range(num_confs):
            with open("apolar_conf"+str(i)+".in",'w') as file:
                for line in og_lines:
                    key_string = "    mol pdb"
                    if line[0:len(key_string)] == key_string:
                        file.write("    mol pdb conf"+str(i)+"_edited.pdb\n")
